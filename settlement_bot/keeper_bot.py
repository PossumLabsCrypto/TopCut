...
# TopCut contract addresses
CONTRACTS = {
    "topcut_market_1": "0x9A5f16c1f2d6b8c9530144aD23Cfa9B3c4717eF1",  
    "topcut_market_2": "0x135a74aaac0E9F4622B94800d069d531d31c4f46",  
    "topcut_market_3": "0x10EF281AAc569Cb011BfcB4e1C6cA490011486a5",  
    "topcut_market_4": "0xB8eC8622D8B7924337CA7B143683459fE5a13f79",  
    "topcut_market_5": "0xE8B9a818D57E2413E05144311E2d4d190c3f711c", 
}
...
def estimate_costs_and_rewards(contract, state):
    ...
    try:
        ...
        max_gas_price = 2.0*1e9  # 2 gwei max
        ...
        # Get actual contract values for accurate reward calculation
        try:
            trade_size = contract.functions.TRADE_SIZE().call()
            share_keeper = contract.functions.SHARE_KEEPER().call()
            share_precision = contract.functions.SHARE_PRECISION().call()
            
            # Calculate keeper reward unit: (TRADE_SIZE * SHARE_KEEPER) / SHARE_PRECISION
            keeper_reward_unit = (trade_size * share_keeper) // share_precision
            min_keeper_reward = int(1e15)  # 0.001 ETH minimum
            
            print(f"Contract values - Trade size: {trade_size/1e18} ETH, "
                  f"Keeper share: {share_keeper}/{share_precision}")
        except Exception as e:
            print(f"Could not get contract values, using defaults: {e}")
            # Fallback to estimated values
            keeper_reward_unit = int(1e16 * 10 / 1000)  # 1e14 per trade
            min_keeper_reward = int(1e15)  # 0.001 ETH minimum
        
        estimated_reward = max(active_cohort_size * keeper_reward_unit, min_keeper_reward)
...
def check_single_contract(contract_info, contract_name, account):
    ...
    try:
        ...
        if can_settle(state):
            ...
            tx_hash = settle_cohort(contract, account, contract_name)
            if tx_hash:
